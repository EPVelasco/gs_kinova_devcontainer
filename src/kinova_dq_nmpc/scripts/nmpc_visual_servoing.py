...
import numpy as np
from kinova_dq_nmpc.nmpc_visual import CameraModel, solverCamera
...
class ZeroCmdFromFeatures:
    def __init__(self):
        ...
        self.N = np.arange(0, self.t_N + self.sample_time, self.sample_time)
        self.N_prediction = self.N.shape[0]

        # Desired velocities camera
        self.ud = np.zeros((6, ))
    ...
    def features_callback(self, msg):
        ...
        for j in range(self.N_prediction):
            uref = self.ud
            aux_ref = np.hstack((desired_camera, uref))
            self.acados_ocp_solver.set(j, "p", aux_ref)

        aux_ref_N = np.hstack((desired_camera, self.ud))
        self.acados_ocp_solver.set(self.N_prediction, "p", aux_ref_N)

        # # Desired Trajectory of the system
        # for j in range(self.N_prediction):
        #     desired_camera = np.zeros((8, ))
        #     uref = self.ud
        #     aux_ref = np.hstack((desired_camera, uref))
        #     self.acados_ocp_solver.set(j, "p", aux_ref)

        # # Desired Trayectory at the last Horizon
        # desired_camera_N = np.zeros((8, ))
        # uref_N = self.ud
        # aux_ref_N = np.hstack((desired_camera_N, uref_N))
        # self.acados_ocp_solver.set(self.N_prediction, "p", aux_ref_N)
        ...
        zero_twist = Twist()
        # zero_twist.linear.x = aux_control[0]
        # zero_twist.linear.y = aux_control[1]
        # zero_twist.linear.z = aux_control[2]

        # zero_twist.angular.x = aux_control[3]
        # zero_twist.angular.y = aux_control[4]
        # zero_twist.angular.z = aux_control[5]
