...
import numpy as np
import casadi as ca
from kinova_dq_nmpc.nmpc_functions import solver
...
# Global variables of GelSight sensor feature
p1 = np.zeros((2,1))
p2 = np.zeros((2,1))
z1 = 0.0
z2 = 0.0
...
def features_callback(msg):
    ...
    try:
        # Extract Point 1 (parte superior derecha)
        p1 = np.array([msg.data[4], msg.data[5]]).reshape(2,1)
        z1 = msg.data[8] * 10.0

        # Extract Point 2 (parte superior izquierda)
        p2 = np.array([msg.data[6], msg.data[7]]).reshape(2,1)
        z2 = msg.data[9] * 10.0

        rospy.loginfo(f"Input features: p1={p1.flatten()}, z1={z1}, p2={p2.flatten()}, z2={z2}")

    except IndexError:
        rospy.logwarn("Error in the features message.")
...
def main():
    ...
    # Subscriber of GelSight feature
    rospy.Subscriber('/gs_feature_coords', Float32MultiArray, features_callback)
    
    
    
    #################### You can use the features global variables of the GS sensor p1 p2 z1 and z2. On line 54, they were initialized
    ...
    omega_control = np.zeros((7, t.shape[0] - N_prediction), dtype = np.double)

    # Emppty current joint values
    q = np.zeros((7, t.shape[0] + 1 - N_prediction), dtype = np.double)
   ...
   # acados_ocp_solver = AcadosOcpSolver(ocp, json_file="acados_ocp_" + ocp.model.name + ".json", build= True, generate= True)
    ...
    for stage in range(N_prediction):
        acados_ocp_solver.set(stage, "u", ud[:, 0])
    ...
    for k in range(0, t.shape[0]-N_prediction):
        ...
        for j in range(N_prediction):
            ...
            thetaref = np.zeros((7, ))
            uref = ud[:,k+j]
            aux_ref = np.hstack((yref, thetaref, uref))
            acados_ocp_solver.set(j, "p", aux_ref)

        # Desired Trayectory at the last Horizon
        yref_N = np.array(dual_d).reshape((8, ))
        thetaref_N = np.zeros((7, ))
        uref_N = ud[:,k+N_prediction]
        aux_ref_N = np.hstack((yref_N, thetaref_N, uref_N))
